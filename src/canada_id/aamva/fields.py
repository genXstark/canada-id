...
FIELD_REGISTRY: dict[str, FieldDef] = {
    ...
    "DBA": _f(
        "DBA", "Document Expiration Date", dtype="N", length=8, req=True
    ),
    "DBD": _f(
        "DBD", "Document Issue Date", dtype="N", length=8, req=True
    ),
    "DDB": _f("DDB", "Card Revision Date", dtype="N", length=8),
    "DDH": _f(
        "DDH", "Date Cardholder Turns 18", dtype="N", length=8
    ),
    "DDI": _f(
        "DDI", "Date Cardholder Turns 19", dtype="N", length=8
    ),
    "DDJ": _f(
        "DDJ", "Date Cardholder Turns 21", dtype="N", length=8
    ),
    # --- Physical description ---
    "DBC": _f(
        "DBC", "Physical Description - Sex", dtype="N", length=1, req=True
    ),
    "DAU": _f(
        "DAU", "Physical Description - Height", length=6, req=True
    ),
    "DAW": _f("DAW", "Physical Description - Weight", dtype="N", length=3),
    "DAY": _f(
        "DAY", "Physical Description - Eye Color", dtype="A", length=3
    ),
    ...
    "DAK": _f(
        "DAK", "Address - Postal Code", length=11, req=True
    ),
    "DCG": _f(
        "DCG", "Country Identification", dtype="A", length=3, req=True
    ),
    # --- Vehicle / endorsement ---
    "DCA": _f(
        "DCA", "Jurisdiction-specific Vehicle Class", "DL", length=6
    ),
    "DCB": _f(
        "DCB", "Jurisdiction-specific Restriction Codes", "DL", length=12
    ),
    "DCD": _f(
        "DCD", "Jurisdiction-specific Endorsement Codes", "DL", length=5
    ),
    ...
    "DCP": _f(
        "DCP", "Jurisdiction-specific Vehicle Classification", length=6
    ),
    "DCQ": _f(
        "DCQ",
    ...
    "DDD": _f(
        "DDD", "Limited Duration Document Indicator", dtype="N", length=1
    ),
    "DDC": _f(
        "DDC", "HazMat Endorsement Expiration Date", dtype="N", length=8
    ),
...
FIELD_ORDER: list[str] = [
    "DCA", "DCB", "DCD", "DCS", "DAC", "DAD", "DBD", "DBB", "DBA",
    "DBC", "DAU", "DAY", "DAZ", "DAG", "DAH", "DAI", "DAJ", "DAK",
    "DAQ", "DCF", "DCG", "DCU", "DAW", "DCE", "DCL", "DCM", "DCN",
    "DCO", "DCP", "DCQ", "DCR", "DCK", "DCI", "DBN", "DBG", "DBS",
    "DDA", "DDB", "DDC", "DDD", "DDE", "DDF", "DDG", "DDH", "DDI",
    "DDJ", "DDK", "DDL", "DCJ",
]
