...
class BarcodeArea:
    ...
    def __init__(self, startBorder: BorderPattern, stopBorder: BorderPattern):
        ...
        # average symbol width of start and stop borders
        self.average_symbol_width = 0.5 * (
            startBorder.average_symbol_width + stopBorder.average_symbol_width
        )
    ...
    def left_x_func_y(self, posY: int) -> int:
        return int(
            self.left_center_x
            + (self.left_delta_x * (posY - self.left_center_y)) / self.left_delta_y
        )

    def right_x_func_y(self, posY: int) -> int:
        return int(
            self.right_center_x
            + (self.right_delta_x * (posY - self.right_center_y)) / self.right_delta_y
        )
